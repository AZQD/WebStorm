...
var dao = require('../dao/mySql_dao.js');
...
function login_post(req, res){
    ...
    if(!userName || !password){
        ...
        if(!isCorrect){
            ...
            //users.queryUserInfo();
            res.redirect('userindex/'+userName);
        }
    }
...
function userIndex(req, res, next){
    ...
    if('wukong' === userName){
        dao.queryAll(function(err, data){
            if(err){
                console.error(err);
                res.end('报错了，这里的内容每个公司不一样');
            }
            else {
                //jobs.ejs是模板，而jobs：data是名字为jobs的数据库
                res.render('jobs.ejs', {jobs:data});
            }
        });
    }else{
        var listLength = 6;
        dao.queryPartial(listLength, function(err, data){
            if(err){
                console.error(err);
                res.end('报错了，这里的内容每个公司不一样');
            }
